...
RSpec.describe "order_items/new", :type => :view do
  ...
  it "renders new order_item form" do
    ...
    assert_select "form[action=?][method=?]", order_items_path, "post" do
      ...
      assert_select "input#order_item_shopify_product_id[name=?]", "order_item[shopify_product_id]"

      assert_select "input#order_item_shopify_variant_id[name=?]", "order_item[shopify_variant_id]"
